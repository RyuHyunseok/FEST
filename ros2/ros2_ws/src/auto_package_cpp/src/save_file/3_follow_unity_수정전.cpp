    ...
    void timer_callback()
    {
        if (path_msg_.poses.size() > 1) {
            ...
            if (dist_to_destination <= destination_radius_) {
                ...
                lfd_ = (odom_msg_.twist.twist.linear.x + lateral_error) * 0.7;  // status_msg_ 대신 odom_msg_ 사용
                ...
                if (is_look_forward_point) {
                    ...
                    // Unity의 180도 회전을 고려하여 yaw 보정
                    double corrected_yaw = robot_yaw_ - M_PI;
                    
                    double cos_yaw = std::cos(corrected_yaw);
                    double sin_yaw = std::sin(corrected_yaw);
                    ...
                    double angular_threshold = M_PI;  // 임계값 설정 (3π/4에서 π/2로 줄임)
