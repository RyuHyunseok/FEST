...
class GoalControlNode(Node):
    def __init__(self):
        ...
        # 구독자 생성
        self.incident_sub = self.create_subscription(
            String,
            '/incidents/new',
    ...
    def incident_callback(self, msg):
        ...
        try:
            # JSON 데이터 파싱
            incident_data = json.loads(msg.data)
            ...
            if 'location' in incident_data:
                ...
                # 새로운 Point 메시지 생성
                goal = Point()
                goal.x = float(location['z'])
                goal.y = - float(location['x'])
                goal.z = 0.0
                
                # 자동 모드 해제
                self.is_auto_mode = False
                self.waiting_for_extinguisher = False
                
                # goal_point 토픽으로 발행
                self.goal_pub.publish(goal)
                self.get_logger().info(f'Published incident goal: ({goal.x}, {goal.y})')
                
        except Exception as e:
            self.get_logger().error(f'Error processing incident data: {e}')
    ...
    def goal_reached_callback(self, msg):
        ...
        if msg.data:
            if not self.is_auto_mode:
                ...
                self.get_logger().info('Reached incident goal, waiting for extinguisher complete')
