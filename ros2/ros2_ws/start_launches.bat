...
@REM start ros2 launch perception_cpp perception_launch.py

rem 2. unity_dijkstra_launch.py 실행(판단/제어 실행)
@REM start ros2 launch auto_driving_cpp unity_dijkstra_launch.py

rem 3. 통신 실행
@REM start ros2 run topic_bridge_py mqtt_ros2_bridge
@REM start ros2 run topic_bridge_py ros2_mqtt_bridge
...
rem 4. 사람/화재 감지 관련 노드들
@REM start ros2 run perception_py opencv_yolo
@REM start ros2 run perception_py fire_image_subscriber
@REM start ros2 run fire_control fire_suppression_node

rem 5. goal_publisher 노드(좌표 입력을 쉽게 하기 위해 따로 실행(1개만))
